	...
	m_staticSprite(staticSprite)
{
	...
	m_textureId = TextureManager::Instance()->Load(m_filePath, WindowManager::Instance()->GetMainWindow()->GetRenderer(), dimensions.x, dimensions.y);
}
...
void SpriteRenderer::Render(glm::vec3 worldPosition)
{
	...
	if (m_staticSprite)
	{
		...
		TextureManager::Instance()->RenderSingle(pCamera, pRenderer, m_textureId, m_screenPosition + cameraPosition, glm::vec2(1.0f), m_dimensions);
		return;
	}
